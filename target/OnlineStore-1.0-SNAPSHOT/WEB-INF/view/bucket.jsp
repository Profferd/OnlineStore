            ...
            <div class="my-sm-3">
                <div class="row justify-content-center">
                    <c:if test="${order.size() <= 0}">
                        <h4 class="text-muted"><fmt:message bundle="${local}" key="bucket.empty"/></h4>
                    </c:if>
                    <table class="table table-hover">
                        ...
                        <tr>
                            ...
                            <th scope="col"><fmt:message bundle="${local}" key="product.price"/></th>
                        ...
                        <c:forEach var="order" items="${order}">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <c:forEach var="product" items="${product}">
                                        <c:forEach var="newPrice" items="${newPrice}">
                                            <c:if test="${product.name == newPrice.key && product.id == order.productId}">
                                                <p class="text-danger"><s><c:out
                                                        value="${product.price}"/> <fmt:message bundle="${local}"
                                                                                                key="price.val"/></s>
                                                    <c:out value="${newPrice.value}"/> <fmt:message bundle="${local}"
                                                                                                    key="price.val"/>
                                                </p>
                                                <c:set scope="request" var="priceFlag" value="1"/>
                                            </c:if>
                                        </c:forEach>
                                        <c:if test="${priceFlag == 0 && product.id == order.productId}">
                                            <p><c:out value="${product.price}"/> <fmt:message bundle="${local}"
                                                                                              key="price.val"/></p>
                                        </c:if>
                                    </c:forEach>
                                </td>
                    ...
                    <c:if test="${order.size() > 0}">
                        ...
                        <h4 class="text-primary"><fmt:message bundle="${local}" key="bucket.pay"/>:
                ...
                <c:if test="${order.size() > 0}">
                    <div class="row justify-content-center">
                        <input type="button" class="btn btn-primary" value="<fmt:message bundle="${local}"
                                                                               key="bucket.arrange"/>"
                               onClick='location.href="${pageContext.request.contextPath}/onlineStore?command=addOrder"'>
                    </div>
                </c:if>
            </div>
        </c:if>
